...
public class CajeroElectronico{
	...
	public void consultarSaldoTarjeta(TarjetaDebito tarjeta, String contrasena){
		if(tarjeta.verificarPassword(contrasena)){
			tarjeta.mostrarSaldo();
			this.registrarLog("CONSULTAR",tarjeta.getNumero(), tarjeta.getSaldo(), "OK:200");
		}else{
			System.out.print("No se puede mostrar el saldo");
			this.registrarLog("CONSULTAR",tarjeta.getNumero(), tarjeta.getSaldo(), "ERROR:403");
		}

    }

	public void cambiarContra(TarjetaDebito tarjeta, String oldPass, String newPass){
		if(tarjeta.verificarPassword(oldPass)){
			tarjeta.setContrasena(newPass);
			
			System.out.println("Cambio de contraseña exitoso");
			this.registrarLog("CAMBIO CONTRASEÑA",tarjeta.getNumero(), 00000000, "OK:200");
		}else{
			System.out.print("Cambio de contraseña erroneo");
			this.registrarLog("CAMBIO CONTRASEÑA",tarjeta.getNumero(), 00000000, "ERROR:403");
		}
	}

	public void transferirDinero(TarjetaDebito tarjeta, int cant_10, int cant_20, int cant_50, int cant_100){
		
		int cantidad_total = (10000*cant_10)+(20000*cant_20)+(50000*cant_50)+(100000*cant_100);
		tarjeta.setSaldo(cantidad_total+tarjeta.getSaldo());
		this.registrarLog("TRANFERIR DINERO",tarjeta.getNumero(), cantidad_total, "OK:200");
		this.cant_10+=cant_10;
		this.cant_20+=cant_20;
		this.cant_50+=cant_50;
		this.cant_100+=cant_100;
		this.cant_dinero_disponible+=cantidad_total;
